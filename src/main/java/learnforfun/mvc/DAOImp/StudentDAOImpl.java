...
public class StudentDAOImpl implements StudentDAO {
    ...
    @Override
    public void insert(Student te) {
        try {
            accountDAO.insert(te.getAccount());
            pstmt = con.prepareStatement("INSERT INTO STUDENT (UID) SELECT UID FROM ACCOUNT WHERE MAIL = ?");
    ...
    public int signInExists(String mail, String password) {
        try {
            pstmt = con.prepareStatement("SELECT STUDENT.UID FROM STUDENT, ACCOUNT WHERE STUDENT.UID = ACCOUNT.UID AND (ACCOUNT.MAIL = ? AND ACCOUNT.PASSWORD = ? )");
    ...
    public int signUpExists(String mail, String username) {
        try {
            pstmt = con.prepareStatement("SELECT STUDENT.UID FROM STUDENT, ACCOUNT WHERE STUDENT.UID = ACCOUNT.UID AND (ACCOUNT.MAIL = ? OR ACCOUNT.USER_NAME = ? )");
            pstmt.setString(1, mail);
    ...
    public Boolean mailExists(String mail) {
        try {
            pstmt = con.prepareStatement("SELECT STUDENT.UID FROM STUDENT, ACCOUNT WHERE STUDENT.UID = ACCOUNT.UID AND ACCOUNT.MAIL = ?");
            pstmt.setString(1, mail);
            ResultSet rs = pstmt.executeQuery();
            return rs.next();
        } catch (SQLException e) {
